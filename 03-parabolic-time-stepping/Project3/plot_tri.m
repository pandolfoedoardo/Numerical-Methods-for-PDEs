function [hs, hc, contour] = plot_tri(pnt, tri, val, mode, levels)
...
%   'nodes_red'   plot black vertices only         (value can be [])
%   'nodes_blue'  plot black vertices only         (value can be [])
%   'node_index'  plot index of each vertex        (value can be [])
%   'node_index_l'plot index of each vertex (large font) (value can be [])
%   'node_label'  plot label of each vertex        (value should be cell array)
%   'edges'       plot black edges only            (value can be [])
%   'edges_red'   plot red edges only              (value can be [])
%   'edges_blue'  plot blue edges only             (value can be [])
%   'edges_t'     plot black thick-line edges only (value can be [])
%   'edges_tred'  plot red thick-line edges only   (value can be [])
%   'edges_tblue' plot blue thick-line edges only  (value can be [])
...
%      h   = text(tri_x, tri_y, tri_z, str, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
      h   = text(tri_x, tri_y, str, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
      h   = text(tri_x, tri_y, str, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
      hs  = [hs; h];
    end
  ...
  case 'edges_red'
    ...
    hs = patch('Vertices', pnt, 'Faces', tri);
    set(hs, 'FaceColor', 'none');
    set(hs, 'EdgeColor', 'red');

  case 'edges_blue'
    % plot the edges of the 2D or 3D triangulation
    hs = patch('Vertices', pnt, 'Faces', tri);
    set(hs, 'FaceColor', 'none');
    set(hs, 'EdgeColor', 'blue');

  case 'edges_t'
    % plot the edges of the 2D or 3D triangulation
    hs = patch('Vertices', pnt, 'Faces', tri);
    set(hs, 'FaceColor', 'none');
    set(hs, 'EdgeColor', 'black', 'LineWidth', 3);
    
  case 'edges_tred'
    % plot the edges of the 2D or 3D triangulation
    hs = patch('Vertices', pnt, 'Faces', tri);
    set(hs, 'FaceColor', 'none');
    set(hs, 'EdgeColor', 'red', 'LineWidth', 3);

  case 'edges_tblue'
    % plot the edges of the 2D or 3D triangulation
    hs = patch('Vertices', pnt, 'Faces', tri);
    set(hs, 'FaceColor', 'none');
    set(hs, 'EdgeColor', 'blue', 'LineWidth', 3);
  ...
  case 'node_index'
    ...
    for node_indx=1:size(pnt,1)
      str = sprintf('%d', node_indx);
      if size(pnt, 2)==2
        h   = text(pnt(node_indx, 1), pnt(node_indx, 2), str, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize',18);
      else
        h   = text(pnt(node_indx, 1), pnt(node_indx, 2), pnt(node_indx, 3), str, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize',18);
      end
      hs  = [hs; h];
    end
